...
def login_screen():
    ...
    loginScreen = Screen(
        800, 600, silver, 'Log In - Tetris en Red', buttons, inputs, text, [], images)
    loginScreen.run()


def main_menu_screen():
    create_game = Button('Crear partida', 200, 310,
                         400, 80, bright_red, red, 6)
    search_game = Button('Buscar partida', 200, 400,
                         400, 80, bright_green, green, 6)
    instructions = Button('Instrucciones', 200, 490, 400,
                          80, bright_blue, blue, 6, instructions_screen)
    about = Button('Configuracion', 100, 620, 200, 60,
                   bright_yellow, yellow, 6, about_screen)
    configuration = Button('Acerca de', 500, 620, 200,
                           60, bright_yellow, yellow, 6)
...
def instructions_screen():
    ...
    t2 = Text(
        'Al pulsar el boton se creara una partida y obtendras el codigo', 12, black, 200, 50)
    t3 = Text(
        'de la misma. Pasaselo a tus amigos para que jueguen contigo.', 12, black, 200, 70)
    t4 = Text('Unirse a una partida', 20, black, 200, 100)
    t5 = Text('Ingresa el codigo de la partida a la que', 12, black, 200, 130)
    t6 = Text('quieres unirte e ingresalo para poder jugar.',
              12, black, 200, 150)
    t7 = Text('Jugando', 20, black, 200, 180)
    t8 = Text('Controla las piezas con las flechas de', 12, black, 200, 210)
    t9 = Text('movimiento y no dejes que toquen el limite superior.',
              12, black, 200, 230)
    text = [t1, t2, t3, t4, t5, t6, t7, t8, t9]

    instructionsScreen = Screen(
        400, 300, silver, 'Instrucciones - Tetris en Red', buttons, [], text)
...
def about_screen():
    ...
    aboutScreen = Screen(
        400, 300, silver, 'Acerca de - Tetris en Red', buttons, [], text)
    aboutScreen.run()
...
def play_screen():
    ...
    p2boardRect = pygame.draw.rect(screen,yellow,(635,80,other_play_width,other_play_height),4)
    draw_playboard(screen,matriz_server,635,80,other_block_size)

    p3boardRect = pygame.draw.rect(screen,yellow,(635,310,other_play_width,other_play_height),4)
    draw_playboard(screen,matriz_server,635,310,other_block_size)

    p4boardRect = pygame.draw.rect(screen,yellow,(635,530,other_play_width,other_play_height),4)
    draw_playboard(screen,matriz_server,635,530,other_block_size)
    '''
    ...
    playScreen = Screen(800, 700, silver, 'Tetris en Red',
                        [], [], text, boards, avatar)
    playScreen.run()
...
def lobby_screen():
    ...
    lobbyScreen = Screen(
        800, 600, silver, 'Lobby - Tetris en Red', buttons, [], text, [], avatar)
    lobbyScreen.run()
