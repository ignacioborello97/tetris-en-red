...
def board_screen(s_width, s_height):
    ...
    avatarRect = pygame.draw.rect(
        screen, yellow, (s_width/16, s_height/7, s_width/5.333, s_height/3.5), 6)

    nextpieceRect = pygame.draw.rect(
        screen, yellow, (s_width/16, s_height*(11/14), s_width/5.333, s_height*(3/14)), 6)

    boardRect = pygame.draw.rect(screen, yellow, ((
        s_width-play_width)/2, (s_height-play_height), play_width, play_height), 6)
    draw_playboard(screen, matriz_server, (s_width-play_width) /
                   2, (s_height-play_height), block_size)

    p2boardRect = pygame.draw.rect(
        screen, yellow, (s_width/1.26, s_height/8.75, other_play_width, other_play_height), 4)
    draw_playboard(screen, matriz_server, s_width/1.26,
                   s_height/8.75, other_block_size)

    p3boardRect = pygame.draw.rect(
        screen, yellow, (s_width/1.26, s_height*(31/70), other_play_width, other_play_height), 4)
    draw_playboard(screen, matriz_server, s_width/1.26,
                   s_height*(31/70), other_block_size)

    p4boardRect = pygame.draw.rect(
        screen, yellow, (s_width/1.26, s_height*(53/70), other_play_width, other_play_height), 4)
    draw_playboard(screen, matriz_server, s_width/1.26,
                   s_height*(53/70), other_block_size)
